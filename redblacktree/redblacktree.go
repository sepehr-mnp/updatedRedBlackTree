...
// /sep
var blockHeight int = 100000

type TwoComparator struct {
	value        int
	lastModified int
}

func TwoIntComparitor(a, b interface{}) int {
	aAsserted := a.(TwoComparator).value - (blockHeight - a.(TwoComparator).lastModified)
	bAsserted := b.(TwoComparator).value - (blockHeight - b.(TwoComparator).lastModified)
	switch {
...
// /sepehrmnp created this!
func NewWithTwoIntComparator() *Tree {
	return &Tree{Comparator: TwoIntComparitor}
}
